...
find . $EXCLUDES -prune -o -type f -exec sed -i \
  -e "s/hmpps-managing-apps-staff-ui/$PROJECT_NAME/g" \
  -e "s/managing-apps-staff-ui/$PROJECT_NAME_WITHOUT_HMPPS/g" \
  -e "s/HMPPS Managing Apps Staff Ui/$PROJECT_DESCRIPTION/g" {} \;

echo "Performing directory renames"

# move helm stuff to new name
mv "helm_deploy/hmpps-managing-apps-staff-ui" "helm_deploy/$PROJECT_NAME"
